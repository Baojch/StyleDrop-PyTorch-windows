...
class MUSE(object):
    ...
    @torch.no_grad()
    def generate(self, config, _n_samples, nnet, decode_fn, is_eval=False,gamma=None, **kwargs):
        ...
        for step in range(_sample_steps):
            ...
            logits = nnet(ids, **kwargs, scale=cfg_scale,gamma=gamma)
            # sampling & scoring
        ...
        for step in range(_sample_steps):
            ...
            annealed_temp = self.gen_temp * (1 - ratio)
            is_mask = (ids == self.mask_ind)
            logits = nnet(ids, **kwargs, scale=cfg_scale,lambdaA=lambdaA,lambdaB=lambdaB,gamma=gamma)
            # sampling & scoring
            sampled_ids = add_gumbel_noise(logits, annealed_temp, device).argmax(dim=-1)
            sampled_logits = torch.squeeze(
                torch.gather(logits, dim=-1, index=torch.unsqueeze(sampled_ids, -1)), -1)
