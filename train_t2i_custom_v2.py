...
def train(config):
    ...
    print(f"dataset:{dataset.object}")
    if(config.data_path == "data_obj/one_obj.json"):
        test_dataset = test_custom_dataset(obj=dataset.object)
        print(f"test dataset:{test_dataset.object}&&&{test_dataset.style}")
        discriptor = Discriptor(dataset.object)
    else:
    ...
    def cfg_nnet(x, context, scale=None,lambdaA=None,lambdaB=None,gamma = None):
