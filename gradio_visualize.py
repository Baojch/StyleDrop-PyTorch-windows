...
num = 0
def cfg_nnet(x, context, scale=None,lambdaA=None,lambdaB=None,gamma = 0.7):
    global num
    print(f"into cfg_nnet:{num}")
    num+=1
    ...
    if lambdaA is None:
        if gamma == 0.:
            ...
            _cond_obj = obj_nnet_ema(x, context=context)
            _cond_w_adapter_obj = obj_nnet_ema(x,context=context,use_adapter=True)
            _uncond_obj = obj_nnet_ema(x, context=_empty_context)
            res1 = _cond_obj + scale * (_cond_obj - _uncond_obj)
            return res1
        else:
            _cond = nnet_ema(x, context=context)
            _cond_w_adapter = nnet_ema(x,context=context,use_adapter=True)
    ...
    if lambdaA is not None:
        ...
        res1 = _cond_w_adapter_obj + lambdaA*(_cond_w_adapter_obj - _cond_obj) + lambdaB*(_cond_obj - _uncond_obj)
        print(f"gamma:{gamma}")
        logits = (1-gamma)*res + gamma*res1
    return logits
...
print(style_adapters)
style_adapters = [os.path.basename(os.path.dirname(x)) for x in style_adapters]
print("style_adapters: ")
print(style_adapters)

obj_adapters = glob("object_adapter/*/")
print(obj_adapters)
obj_adapters = [os.path.basename(os.path.dirname(x)) for x in obj_adapters]
print("obj_adapters: ")
print(obj_adapters)
...
obj_nnet_ema = train_state.obj_nnet_ema
obj_nnet_ema.eval()
obj_nnet_ema.requires_grad_(False)
obj_nnet_ema.to(device)

style_ref = {
    "None": None,
    # **{x: os.path.join("style_adapter", x, "adapter.pth") for x in style_adapters}
    **{x: glob(os.path.join("style_adapter", x, "*.pth")) for x in style_adapters}
}
style_postfix = {
    "None": "",
...
obj_ref = {
    "None": "",
    # **{x: os.path.join("style_adapter", x, "adapter.pth") for x in style_adapters}
    **{x: glob(os.path.join("object_adapter", x, "*.pth")) for x in obj_adapters}
}
obj_postfix = {
    "None": "",
    **{x: f"a {x.replace('_', ' ')} " for x in obj_adapters}
}
def decode(_batch):
    return vq_model.decode_code(_batch)

def process(prompt,num_samples,lambdaA,lambdaB,gamma,style,seed,sample_steps,object):
    ...
    if object!="None":
        adapter_obj_path = obj_ref[object]
        adapter_obj_postfix = obj_postfix[object]
        adapter_obj_path = adapter_obj_path[0]
        print(f"load adapter path: {adapter_obj_path} with postfix: {adapter_obj_postfix}")
        if adapter_obj_path is not None:
            obj_nnet_ema.adapter.load_state_dict(torch.load(adapter_obj_path))
        print("load OBJ adapter Done!")
    ...
    # Encode prompt
    prompt = adapter_obj_postfix+prompt+adapter_postfix
    ...
    print(f"lambdaA: {lambdaA}, lambdaB: {lambdaB}, sample_steps: {sample_steps}, gamma: {gamma}")
    if seed==-1:
        seed = random.randint(0,65535)
    config.seed = seed
    print(f"seed: {seed}")
    set_seed(config.seed)
    res = muse.generate(config,num_samples,cfg_nnet,decode,is_eval=True,gamma=gamma,context=text_embedding)
    print(res.shape)
...
with block:
    with gr.Row():
        gr.Markdown("## StyCraft: Object-Driven Text-to-image Generation in Any Style")
    with gr.Row():
        with gr.Column():
            prompt = gr.Textbox(label=f"**Prompt** a [selected object] + your prompt + in [selected style]") 
            run_button = gr.Button(value="Run")
            num_samples = gr.Slider(label="Images", minimum=1, maximum=12, value=1, step=1)
            seed = gr.Slider(label="Seed", minimum=-1, maximum=2147483647, step=1, value=-1)
            style = gr.Radio(choices=style_adapters+["None"], type="value",value="None",label="Style")
            object = gr.Radio(choices=obj_adapters+["None"], type="value",value="None",label="Object")
            ...
            with gr.Accordion("Advanced options",open=False):
                ...
                gamma = gr.Slider(label="gamma[0(style)~1(object)]", minimum=0.0, maximum=1.0, value=0.5, step=0.1)
    ...
    ips = [prompt,num_samples,lambdaA,lambdaB,gamma,style,seed,sample_steps,object]
    run_button.click(
        fn=process,
        inputs=ips,
